...
/*WhatsAlexa.addCommand({pattern: 'stam$', fromMe: true, desc: Lang.STAM_DESC}, (async (message, match) => {
    ...
    if (message.reply_message !== true && match[1] !== '') {
            grup = await message.client.groupMetadata(message.jid);
            var jids = [];
            mesaj = '';
            grup['participants'].map(async (uye) => {
              await message.sendReply(uye.jid, match[1]);
    } else if (match[1] === '' && message.reply_message !== false) {
            grup = await message.client.groupMetadata(message.jid);
            var jids = [];
            mesaj = '';
            grup['participants'].map(async (uye) => {
              await message.sendReply(uye.jid, message.reply_message.text);
    } else {
